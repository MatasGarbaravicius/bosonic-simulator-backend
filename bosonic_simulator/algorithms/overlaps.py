...
import bosonic_simulator.utils.block_diagonal_symplectic_form_utils as omega
...
def overlaptriple(
    ...
    * Computing expressions, such as determinants, in log-space (e.g., using `numpy.linalg.slogdet`)
    ...
    omega_dhat = omega.multiply_vector(
        coherent_state_utils.displacement_vector(lambda_)
    )  # omega * dhat(lambda)
...
def overlap(
    ...
    gamma2_p = gamma1
    gamma3_p = gamma2
    d1_p = coherent_state_utils.displacement_vector(alpha1)
    d2_p = coherent_state_utils.displacement_vector(alpha1)
    d3_p = coherent_state_utils.displacement_vector(alpha2)
